...
class AdversarialEnv(multigrid.MultiGridEnv):
  ...
  def step_adversary(self, loc):
    ...
    return obs, 0, done, ({"agent_pos": self.agent_start_pos, "agent_dir": self.agent_start_dir, "goal_pos": self.goal_pos, "obs": image, "render": self.render(mode="rgb_array")} if done else {})
  ...
  def reset_random(self):
    ...
    action_dist = torch.distributions.categorical.Categorical(
        torch.ones(self.adversary_action_dim) / self.adversary_action_dim)
    tmp_adversary_max_steps = self.adversary_max_steps
    self.adversary_max_steps = round(self.n_clutter/2) + 2
    for step in range(round(self.n_clutter/2) + 2):
        action = action_dist.sample()
        obs, reward, done, infos = self.step_adversary(action)
